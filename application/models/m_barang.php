...
class M_barang extends CI_Model
{
	...
	function show_stokmasuk()
	{
		$hasil = $this->db->query("SELECT bm.id, bm.tanggal_transaksi, b.kodebarang, namabarang, jumlah_masuk, p.kode_supplier, p.nama_supplier
								FROM barangmasuk bm, barang b, supplier p
	...
	{
		$hasil = $this->db->insert('barangmasuk', $data);
		return $hasil;
	}

	function hapus_stok($kode)
	{
		$deleteStok = $this->db->where('id', $kode)->get('barangmasuk')->row();
		...
		$hasil = $this->db->where('id', $kode)->delete('barangmasuk');
		return $hasil;
	}
	...
	function totalStok_byDate($date)
	{
		...
		return $this->db->get('barangmasuk')->row()->jumlah_masuk;
	}
